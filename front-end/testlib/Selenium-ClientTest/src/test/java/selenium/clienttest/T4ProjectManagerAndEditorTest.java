...
public class T4ProjectManagerAndEditorTest {
...
//insert attribute testAttr1:int
    ...
    @Test//case 10 Create NoteBox
    public void testCreateNoteBoxAndEditIt() throws Exception {
        TestUtils.createProject("case10project");
        // actions.doubleClick(TestUtils.getElemInManagerByName("v baseFolder")).perform();
        Thread.sleep(500);

        driver.findElement(By.xpath("//div[@id='tools']/div[2]/div/div/div[3]")).click();//new notebox on toolbox
        Thread.sleep(500);
        actions.dragAndDropBy(driver.findElement(By.xpath("//div[@id='dgbox']")), 10, -15);//draw the box

        actions.build().perform();
        Thread.sleep(1000);
        actions.doubleClick(driver.findElement(By.xpath("//div[@id='dgbox']/div/div/app-note-box/app-diagram-object/div/div/div/div[2]"))).perform();//dblclick on title to change it
        Thread.sleep(1500);
        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).click();
        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).clear();
        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).sendKeys("Test");
        actions.keyDown(Keys.SHIFT)
                .sendKeys(Keys.ENTER)
                .build()
                .perform();
           actions.keyUp(Keys.SHIFT).keyUp(Keys.SHIFT).build().perform();
        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).sendKeys("NewLine");
        actions.keyDown(Keys.SHIFT)
                .sendKeys(Keys.ENTER)
                .build()
                .perform();
        ...
        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).sendKeys("$@.áűőóüöíé123");

        Thread.sleep(500);

        driver.findElements(By.xpath("//textarea[@name='text']")).get(0).sendKeys(Keys.ENTER);
        Thread.sleep(1000);
        Assertions.assertEquals("Test\nNewLine\n$@.áűőóüöíé123",
                driver.findElement(By.xpath("//div[@id='dgbox']/div/div/app-note-box/app-diagram-object/div/div/div/div[2]")).getText());
