...
//+service.getSessionStateById(action.getTarget().getTarget_id()).getLockerUser_id()+" has lock on this.\n object restored.");
                    ...
                    //SERVER_SIDE ACTIONS ////////////////////////////////////////////
                         ...
                         case S_USER_CONNECT:
                List<SessionUserDto> dtos= service.tokenToSession(action.getSession_jwt()).getUserSockets().stream().map(u->{
                     SessionUserDto su=new SessionUserDto();
                     su.setUser(new User_PublicDto(service.getUserEntityById(u.getUser_id())));
                     su.setColor(u.getColor());
                     return su;
                 }).collect(Collectors.toList());
                           action.setAction(ACTION_TYPE.SESSION_USER_LIST_UPDATE);
                action.setJson(mapper.writeValueAsString(dtos));
                this.sendAll(action, Q.ACTION);
                    ...
                    try {
                        ...
                        List<SessionUserDto> dtos2 = service.tokenToSession(action.getSession_jwt()).getUserSockets().stream().map(u -> {
                            SessionUserDto su = new SessionUserDto();
                            su.setUser(new User_PublicDto(service.getUserEntityById(u.getUser_id())));
                            su.setColor(u.getColor());
                            return su;
                        ...
                        action.setAction(ACTION_TYPE.SESSION_USER_LIST_UPDATE);
                        action.setJson(mapper.writeValueAsString(dtos2));
                        this.sendAll(action, Q.ACTION);
                        break;
...
//METHODS_BEG
    ...
    protected Integer getLockerIdIfexists(EditorAction action, Integer target_id) throws NotFoundException {
        try {
    ...
    protected void sendAll(EditorAction action, Q queue) throws JwtParseException, NotFoundException {
        switch (queue) {
            case STATE:
    ...
    protected void sendBackPrivate(EditorAction action, Q queue, String msg) throws JwtParseException, NotFoundException {
        switch (queue) {
            case STATE:
                /*   SessionStateResponse resp = new SessionStateResponse(
                        service.tokenToSession(action.getSession_jwt()).getSessionStateById(action.getTarget().getTarget_id()),
     ...
     */
    protected void sendSimpleClassDeleteRestoreMessage(SimpleClass obj, EditorAction action, String message) throws JwtParseException, NotFoundException {
    ...
    protected void sendCustomMessage(Integer target_object_id, EditorAction action, Q queue, TARGET_TYPE target_type, RESPONSE_SCOPE response_scope, String message) throws JwtParseException, NotFoundException {
        switch (queue) {
            case ACTION:
    ...
    /*send it to all*/
    protected void sendCustomMessage(EditorAction action, Q queue, TARGET_TYPE target_type, RESPONSE_SCOPE response_scope, String message) throws JwtParseException, NotFoundException {
